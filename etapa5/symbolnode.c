...
SymbolNode *SymbolNode_create_with_new_symbol(const char *_key, int _line, int _nature, int _type, const char *_value) {
    ...
    new_node->symbol = Symbol_create(_key, _line, _nature, _type, _value);
    new_node->next = NULL;
    return new_node;
}
...
void SymbolNode_test_implementation() {
    printf("--------------- Tests for %s implementation: Running now ---------------\n", __FILE__);
        ...
        Symbol *symbol_test_3 = Symbol_create(key_test_3, 3, 30, 300, value_test_3);
        ...
        printf("Adding another Symbol at the end of the list:\n");
        symbolnode_test = SymbolNode_add_node_at_the_end(symbolnode_test, symbol_test_3);
        SymbolNode_print(symbolnode_test);
